...
const INTERMEDIARY_KEY: &[u8] = include_bytes!("certs/intermediary.key");
const INTERMEDIARY_CERT: &[u8] = include_bytes!("certs/intermediary.crt");
const ROOT_CERT: &[u8] = include_bytes!("certs/root.crt");
...
fn main() {
    ...
    // We create the issuer signer using the x5chain builder.
    let issuer_signer =
        Es256SignerWithChain::generate("issuer".to_string(), None, &x5chain_builder).unwrap();
